...
const Chat_image = ({ loggedInUserId, activeSessionId, setActiveSessionId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("ID phiên chat hiện tại (trước khi gửi):", activeSessionId);
    ...
    try {
      const response = await fetch('http://localhost:3001/api/generate-image', {
        ...
        body: JSON.stringify({
          ...
          session_id: activeSessionId // Gửi activeSessionId, có thể là null/undefined nếu là lần đầu
        }),
      });
      ...
      setDisplayedImageUrl(`data:image/png;base64,${data.image_base64}`);
      setDisplayedPrompt(prompt); // Lưu prompt đã dùng để hiển thị
      setPrompt(''); // Xóa prompt trong ô nhập liệu sau khi gửi

      // QUAN TRỌNG: Cập nhật activeSessionId nếu backend trả về một ID mới
      ...
      console.log("Ảnh đã được tạo và lịch sử chat đã cập nhật. Session ID:", data.session_id);
    ...
    <div className="Form_Image">
      ...
      {displayedImageUrl && (
        <div className="image-result">
          ...
          <img src={displayedImageUrl} alt="Generated Content" />
        </div>
      )}
    </div>
  );
